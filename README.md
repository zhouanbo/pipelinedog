...
As soon as you execute PipelineDog, you are greeted be a welcome screen where you can click either "New Project" to create a new pipeline project, or "Open Project" to open an existing pipeline project. The app need a work directory to create sub-folders and file lists. Therefore, a folder selection dialog will pop-out if you choose to create a new project.

[[https://github.com/zhouanbo/pipelinedog/blob/gh-pages/p1.png|alt=p1]]

After selecting a work directory for the app, an input list will be created inside the folder
If you chose to create a new project, a typcial empty project screen in the map mode will be shown:

[[https://github.com/zhouanbo/pipelinedog/blob/gh-pages/p2.png|alt=p2]]
...
The "Header panel" lists actions to perform project-wise.

- ***Create, Open or save project***: Click "New" button (or CmdOrCtrl+N) to create a new project. Click "Open" button (or CmdOrCtrl+O) to open a new project. Click the "Save" button  (or CmdOrCtrl+S) to save the project.
- ***Import File***: Click "Import File" button to choose the files to import to the "File panel".
- ***Switch modes***: Click the "Map / Code" button to switch back and forth.
- ***Parse all tools***: Click the "Parse All" button to parse all the tools. This will report any syntax error if unsuccessful, or create output lists for each of tools if successful.
- ***Export command***: Click the "Export" button in the header panel to export the final parsed pipeline command to a BASH file.
- ***Run command***: Click the "Run" button in the header panel to open a new window and directly run the final parsed pipeline command inside.

**Tool Panel**

The "Tool panel" lists all tools in the pipeline with their names and descriptions.

- ***Edit Tool***: Click a tool tab to switch to and edit that tool.

**File Panel**

The "File panel" lists all the input and output lists generated by the app, in addition to the user imported files.

- ***View file content***: View the file content by clicking the "eye" icon in the file panel.
- ***Delete file***: Click the "delete" icon to remove the file from the file panel. It only removes the file reference in the app, not the actual file.
- ***Quick insert file path***: While defining the tool, click the file names in the file panel to insert the file path to your cursor's position. 

**Map Panel**

You are in "Map Mode" therefore is shown the "Map panel" by default when you first create a new project. You can see the app have a default tool named "New Step" created for you already. In "Map panel", you can organzie your pipeline steps. 

***Create Step***: You can create a step in the same hierarchy by clicking the arrow pointing to the right of the default step, or create a step in the lower hierarchy by clicking the arrow pointing downwards. 
***Delete Step***: You can select a step by single click it once, and selected step will turn black. You can then delete it by clicking the red "Delete" button on the top right cornor of the panel. Or unselect the step by click it once more.
***Edit Step***: To edit a step, you need to enter "Tool Mode" from the current "Map Mode". You can do this by either by double click a tool node in the "Map panel" or single click a tool tab in the "Tool panel".

**Tool Panel**

By double clicking a tool node or single clicking a tool tab, you can enter the "Tool Mode". The tool panel has a profile section and a code section:

[[https://github.com/zhouanbo/pipelinedog/blob/gh-pages/p3.png|alt=p3]]

- ***Edit tool profile***: You can edit the tool's name and description by editing the values in the input boxes in the profile section, or by directly editing the JSON properties in the code section.
- ***Define tool***: You can define the tool using a JSON object inside the code editor.
- ***Parse tool***:  Click "Parse" button in the "code actions" section to parse it into BASH commands and create respective output list, and click "Editor" button to switch back and continue editing the code.
- ***Export tool***: Click "Export" button to export your tool's definition into an external file.
- ***Import tool***: Click "Import" button to import a saved tool definition to REPLACE the current tool.
